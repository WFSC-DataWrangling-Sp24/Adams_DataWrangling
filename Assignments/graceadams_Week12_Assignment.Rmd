...
lengths <- c(10.44, 23, 3.45)
species <- c("Stegosauria", "Theropoda", "Sauropoda")

results_2a <- vector(mode = "numeric", length = length(lengths))
...
for (i in seq_along(lengths)) {
  ...
  for (j in seq_along(species)) {
    species_j <- species[j]
    est_mass <- mass_from_length(length_i, species_j)
    results_2a[(i - 1) * length(species) + j] <- est_mass
  }
}
