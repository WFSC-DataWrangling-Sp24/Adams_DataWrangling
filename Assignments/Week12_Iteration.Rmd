...
```{r}
# create a vector of volumes to feed through loop
volumes <- c(1.6, 3, 8)

# start on the for loop. for each volume in the vector of volumes
...
# Option 2: name specific columns with the $ then subset by index (i)
for (i in 1:nrow(vol_data)) {
...
download.file("http://www.datacarpentry.org/semester-biology/data/locations.zip",
              "../Assignments/data_raw/locations.zip")

unzip(zipfile = "../Assignments/data_raw/locations.zip", exdir =  "../Assignments/data_raw")
```
...
for (i in 1:num_files){
  ...
  # save the number of rows per file
  results[i] <- nrow(data)
}
