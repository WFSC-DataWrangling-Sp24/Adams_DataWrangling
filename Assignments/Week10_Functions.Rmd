...
```{r}
# calculate a volume
calc_volume <- function(length, width, height) {
  area <- length*width
  volume <- area*height
  return(volume)
}
...
calc_volume(length = 2, width = 4, height = 3.5)
calc_volume(2, 4, 3.5)
```
...
```{r}
first_volume <- calc_volume(2, 4, 3.5)
```
...
```{r}
# calculate a volume
calc_volume <- function(length, width, height= 2) {
  area <- length*width
  volume <- area*height
  return(volume)
}

# length 0.8, width, 1.6
calc_volume(0.8, 1.6)
calc_volume(0.8, 1.6, 2)
calc_volume(0.8, 1.6, 4)
```
...
```{r}
calc_mass_from_volume <- function(volume) {
  mass <- 2.65 * volume^0.9
  return(mass)
}
calc_mass_from_volume(4)

shrub_vol <- calc_volume(0.8, 1.6)
shrub_mass <- calc_mass_from_volume(shrub_vol)
```
...
shrub_mass <- calc_volume(0.8, 1.6) %>% 
  calc_mass_from_volume()
shrub_mass
```
...
make_hist(penguins, body_mass_g, "Body Mass (g)")
```
...
```{r}
make_hist <- function(df, column, label_x, fill_col) {
  ggplot(df, aes(x = {{column}}, fill = {{fill_col}})) +
    geom_histogram() +
    labs(x = label_x)
}

make_hist(penguins, body_mass_g, "Body Mass (g)", species)
make_hist(penguins, flipper_length_mm, "Flipper Length (mm)")
```
