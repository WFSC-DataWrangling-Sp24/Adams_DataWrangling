...
height_combined <- get_height_from_weight(weight_grams, b=0.32)
height_combined
...
get_mass_from_length(12, a = 10.95, b=2.64)

mass_from_length <- get_mass_from_length(12, a = 10.95, b=2.64)
mass_in_pounds <- convert_kg_to_pounds(mass_from_length)
mass_in_pounds
```
...
count_data <- data.frame(state_vector, site_vector, count_vector, area_vector)

colnames(count_data) <- c("state","site","count","area")
...
```{r}
make_plot <- function(df, column, label_x, label_y) {
  ggplot(df, aes(x=area, y=count, color= {{column}}), size =3) +
    geom_point()+
    labs(x=label_x, y=label_y)
}

make_plot(count_data, state, "Area [ha]", "Number of Individuals")
```
...
```{r}
ggsave("/Users/graceadams/Documents/Git/Adams_DataWrangling/output/question_6b.png")
```
...
```{r}
make_plot(count_data, site, "Area [ha]", "Number of Individuals")
```
...
```{r}
ggsave("/Users/graceadams/Documents/Git/Adams_DataWrangling/output/question_6c.png")
```
