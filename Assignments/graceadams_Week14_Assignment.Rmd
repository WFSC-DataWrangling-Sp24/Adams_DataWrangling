...
```{r}
sqrt_seq_list <- lapply(seq_list, sqrt)

sqrt_seq_list <- do.call(rbind, sqrt_seq_list)
...
sqrt_seq_list <- mclapply(seq_list, sqrt, mc.cores = nCores)
...
```{r}
results_3a <- foreach::foreach(i = 1:nrow(dinosaur_lengths), .combine = c) %do% {
...
output <- foreach::foreach(i = 1:nrow(dinosaur_lengths), .combine = c) %dopar% {
  mass_from_length(dinosaur_lengths$lengths[i], dinosaur_lengths$species[i])
}
...
length_forest <- length(forest$countryname)

results_4a <- data.frame(countryname = vector(mode= "character", length = length_forest),
                         change_in_forest = vector(mode= "numeric", length = length_forest))

change_in_forest <- vector(mode= "numeric", length = length_forest)

for (i in 1:length(forest)) {
  results_4a$countryname[i] <- forest$countryname[i]
  results_4a$change_in_forest[i] <- forest$'2021'[i] - forest$'1990'[i]
  
  change_in_forest[i] <- results_4a$change_in_forest[i]
}
...
(b) Perform the same task as in (a) using the `foreach` function and the `%do%` operator. Use the `.combine = c` argument to save the output as a vector

```{r}
results_4b <- foreach::foreach(i = 1:nrow(forest), .combine = c) %do% {
...
output <- foreach::foreach(i = 1:nrow(forest), .combine = c) %dopar% {
  forest$'2021'[i] - forest$'1990'[i]
}
